...
<head>
    ...
    <title>@yield('title', 'Portal Pembayaran Siswa - SMK Muhammadiyah Sekampung')</title>
...
<body class="bg-blue-50/40 text-slate-800 font-sans min-h-screen flex flex-col antialiased">
    ...
    <header class="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 border-b border-blue-700/50 text-white sticky top-0 z-40 shadow-lg shadow-blue-900/15">
                ...
                <div class="flex items-center gap-2 sm:gap-6">
                    <nav class="hidden md:flex items-center gap-1.5">
                        <a href="{{ route('siswa.dashboard') }}" 
                           class="px-3.5 py-2 rounded-xl text-xs font-bold transition {{ request()->routeIs('siswa.dashboard') ? 'bg-white text-blue-900 shadow-md' : 'text-blue-100 hover:text-white hover:bg-white/10' }}">
                            <i class="fa-solid fa-house mr-1.5"></i> Dashboard
                        </a>
                        <a href="{{ route('siswa.tagihan') }}" 
                           class="px-3.5 py-2 rounded-xl text-xs font-bold transition {{ request()->routeIs('siswa.tagihan') || request()->routeIs('siswa.bayar.*') ? 'bg-white text-blue-900 shadow-md' : 'text-blue-100 hover:text-white hover:bg-white/10' }}">
                            <i class="fa-solid fa-file-invoice-dollar mr-1.5"></i> Tagihan Saya
                        </a>
                        <a href="{{ route('siswa.riwayat') }}" 
                           class="px-3.5 py-2 rounded-xl text-xs font-bold transition {{ request()->routeIs('siswa.riwayat') ? 'bg-white text-blue-900 shadow-md' : 'text-blue-100 hover:text-white hover:bg-white/10' }}">
                    ...
                    <div class="flex items-center gap-3 border-l border-blue-700/60 pl-3 sm:pl-6">
                        <div class="hidden sm:block text-right">
                            ...
                            <p class="text-[10px] text-blue-200">NISN: {{ Auth::user()->siswa?->nisn ?? '-' }}</p>
                        </div>
        ...
        <div class="md:hidden bg-blue-950 border-t border-blue-900 px-4 py-2 flex justify-around text-xs">
            <a href="{{ route('siswa.dashboard') }}" class="flex flex-col items-center gap-1 py-1 {{ request()->routeIs('siswa.dashboard') ? 'text-white font-bold' : 'text-blue-300' }}">
                <i class="fa-solid fa-house"></i> Beranda
            </a>
            <a href="{{ route('siswa.tagihan') }}" class="flex flex-col items-center gap-1 py-1 {{ request()->routeIs('siswa.tagihan') ? 'text-white font-bold' : 'text-blue-300' }}">
                <i class="fa-solid fa-file-invoice-dollar"></i> Tagihan
            </a>
            <a href="{{ route('siswa.riwayat') }}" class="flex flex-col items-center gap-1 py-1 {{ request()->routeIs('siswa.riwayat') ? 'text-white font-bold' : 'text-blue-300' }}">
    ...
    <footer class="bg-white border-t border-blue-100 py-6 text-center text-xs text-slate-500">
        <div class="max-w-7xl mx-auto px-4">
            ...
            <p class="text-[11px] text-slate-400 mt-2">&copy; {{ date('Y') }} SMK Muhammadiyah Sekampung. All rights reserved.</p>
        </div>
    </footer>
