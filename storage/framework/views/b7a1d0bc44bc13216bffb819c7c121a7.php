<?php $__env->startSection('title', 'Checkout Pembayaran Transfer - SMK Muhammadiyah Sekampung'); ?>
...
<div class="max-w-4xl mx-auto space-y-6" x-data="checkoutApp()">
    ...
    <div class="flex items-center justify-between">
        <a href="<?php echo e(route('siswa.tagihan')); ?>" class="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-slate-800 transition">
            <i class="fa-solid fa-arrow-left"></i> Kembali ke Daftar Tagihan
        </a>
        <span class="text-xs font-semibold text-slate-400">Tahun Ajaran: <?php echo e($tagihan->tahun_ajaran); ?></span>
    </div>
    ...
    <div class="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 text-white rounded-3xl p-6 sm:p-8 shadow-xl shadow-blue-900/10 border border-blue-700/60">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div class="bg-white/10 backdrop-blur border border-white/20 p-4 rounded-2xl text-right flex-shrink-0">
                ...
                <span class="text-xl font-black text-white">Rp <?php echo e(number_format($tagihan->sisa_tagihan, 0, ',', '.')); ?></span>
            </div>
        </div>
    </div>

    <!-- Form Checkout & Upload Bukti -->
    <form action="<?php echo e(route('siswa.bayar.upload', $tagihan->id)); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
        <?php echo csrf_field(); ?>
        ...
        <div class="bg-white rounded-3xl p-6 sm:p-8 border border-blue-100 shadow-sm space-y-6">
            ...
            <div class="space-y-3">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $tagihan->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->sisa_pos > 0): ?>
                        <div class="p-4 rounded-2xl border transition-all"
                             :class="selectedItems['<?php echo e($item->id); ?>'] ? 'bg-blue-50/70 border-blue-300 shadow-sm' : 'bg-slate-50 border-slate-200 opacity-90'">
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                
                                <div class="flex items-start gap-3">
                                    <input type="checkbox" 
                                           id="pos_<?php echo e($item->id); ?>" 
                                           class="mt-1 w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500 cursor-pointer"
                                           x-model="selectedItems['<?php echo e($item->id); ?>']"
                                           @change="onCheckChange('<?php echo e($item->id); ?>', <?php echo e((float)$item->sisa_pos); ?>)">
                                    <div>
                                        <label for="pos_<?php echo e($item->id); ?>" class="text-sm font-bold text-slate-900 cursor-pointer">
                                            <?php echo e($item->posPembayaran?->nama_pos); ?>

                                        </label>
                                        <div class="flex items-center gap-3 text-xs text-slate-500 mt-0.5">
                                            <span>Sisa Tanggungan: <strong class="text-blue-700">Rp <?php echo e(number_format($item->sisa_pos, 0, ',', '.')); ?></strong></span>
                                        </div>
                                    </div>
                                </div>

                                <div class="w-full sm:w-60" x-show="selectedItems['<?php echo e($item->id); ?>']" x-transition>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" 
                                               name="alokasi[<?php echo e($item->id); ?>][nominal]" 
                                               x-model.number="nominalItems['<?php echo e($item->id); ?>']"
                                               :disabled="!selectedItems['<?php echo e($item->id); ?>']"
                                               max="<?php echo e($item->sisa_pos); ?>"
                                               min="0"
            ...
            <div class="bg-gradient-to-br from-blue-900 to-indigo-900 text-white p-5 rounded-2xl space-y-3 border border-blue-800 shadow-md">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
                    <div>
                        ...
                        <span class="font-mono text-lg font-black text-white tracking-wider"><?php echo e($setting->nomor_rekening ?? '7123456789'); ?></span>
                    </div>
                    <button type="button" onclick="navigator.clipboard.writeText('<?php echo e($setting->nomor_rekening ?? '7123456789'); ?>'); alert('Nomor rekening disalin!');" 
...
<?php echo $__env->make('layouts.siswa', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\web skripsi\resources\views/siswa/bayar.blade.php ENDPATH**/ ?>